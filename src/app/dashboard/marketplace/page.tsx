...
function MarketplaceContent() {
  ...
  const handleBulkPublish = async (published: boolean) => {
    ...
    if (ok > 0) {
      setProducts(prev => prev.map(p => selected.has(p.id) ? { ...p, published } : p))
      success(published ? `${ok} item${ok === 1 ? '' : 's'} published` : `${ok} item${ok === 1 ? '' : 's'} hidden`)
    } else {
  ...
  const handleBulkDelete = async () => {
    ...
    if (ok > 0) {
      ...
      success(`Deleted ${ok} item${ok === 1 ? '' : 's'}`)
    } else {
    ...
    <div className={styles.page}>
      ...
      <BulkBar
        ...
        actions={[
          ...
          { label: 'Delete', icon: '🗑', onClick: () => setConfirmBulkDelete(true), disabled: bulkBusy, danger: true, title: 'Delete selected items' },
        ]}
      />
      ...
      {filteredProducts.length === 0 && !showProductForm ? (
        ...
        <div className={styles.list}>
          {filteredProducts.map(product => (
            <div key={product.id} className={`${styles.item} ${selected.has(product.id) ? styles.itemSelected : ''}`}>
      ...
      <ConfirmDialog
        ...
        message="This permanently removes the selected products and their associated data. This cannot be undone."
        confirmLabel="Delete"
        variant="danger"
      />
    </div>
