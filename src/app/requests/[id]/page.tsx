...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  if (!request) return {}
  const title = `${request.title} — Requests — XistrYmemZ`
  const description = request.description?.slice(0, 160) || 'A community request on XistrYmemZ'
  return {
    title,
    description,
    openGraph: { title, description, images: request.imageUrl ? [request.imageUrl] : [] },
